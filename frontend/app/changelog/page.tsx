...
const RELEASES: Release[] = [
  {
    ...
    date: "June 2026",
    tag: "latest",
    summary: "2FA enforcement, API Keys, Session Management, Share Links, Comments, Bulk Ops, Templates, Subscriptions, Connectors, Scheduled Reports, WebSocket notifications, and full Swagger coverage.",
    changes: [
      { type: "feature",     text: "2FA enforced at login — POST /auth/login returns 403 {code: '2fa_required', user_id} when TOTP is enabled; complete with POST /auth/2fa/validate" },
      { type: "feature",     text: "POST /auth/2fa/validate — new endpoint to complete 2FA login and receive JWT tokens" },
      { type: "feature",     text: "UserSession created on every login — Sessions tab in Settings now shows real devices with IP and user-agent" },
      { type: "feature",     text: "API Keys — full CRUD in Settings (sk_ prefix, scopes, expiry, last_used_at); X-API-Key header accepted as JWT alternative" },
      { type: "feature",     text: "Document Share Links — time-limited, password-protected, max-view-count links; public resolve endpoint (no auth)" },
      { type: "feature",     text: "Document Comments — threaded replies, edit/delete, batch user name resolution" },
      { type: "feature",     text: "Bulk document operations — bulk-delete, bulk-tag, bulk-signature-status (up to 50 docs)" },
      { type: "feature",     text: "Document Templates — pre-built SME templates with {{placeholder}} filling; usage_count tracking" },
      { type: "feature",     text: "Subscriptions — Stripe Checkout + billing portal + webhook handler; Free/Starter/Professional/Enterprise plans" },
      { type: "feature",     text: "Connectors — QuickBooks Online, Xero, MTN MoMo webhook, Airtel Money webhook" },
      { type: "feature",     text: "Scheduled reports — cron-based email delivery via Celery beat; UI in Analytics page" },
      { type: "feature",     text: "Data retention — Celery beat daily task archives documents/knowledge entries per company retention_days setting" },
      { type: "feature",     text: "WebSocket LiveNotificationBanner — mounted in dashboard layout; notifications page subscribes to live WS updates" },
      { type: "improvement", text: "Swagger — all 20 tag groups with full descriptions; X-API-Key security scheme added alongside BearerAuth" },
      { type: "improvement", text: "Public endpoints (login, register, 2fa/validate, share-links/{token}, MoMo/Airtel webhooks) correctly marked security: [] in OpenAPI" },
      { type: "improvement", text: "API Reference page updated with all 14 endpoint groups and 80+ endpoints" },
      { type: "improvement", text: "GET /documents/{id}/download now returns both url and download_url keys for frontend compatibility" },
      { type: "improvement", text: "S3/MinIO — all boto3 clients read S3_ENDPOINT_URL from config; .env updated with MinIO defaults" },
      { type: "improvement", text: "Admin audit log — action + resource type filters, CSV export button, shows X of Y count" },
      { type: "improvement", text: "Analytics page — Schedule Report inline form with cron presets and email input" },
      { type: "fix",         text: "animate-slide-in CSS class added to globals.css — toast notifications now animate correctly" },
      { type: "fix",         text: "UserSession — added user backref relationship; User model has sessions relationship" },
      { type: "security",    text: "2FA validate endpoint is public (no auth required) — correctly excluded from BearerAuth requirement" },
    ],
  },
  {
    version: "1.5.0",
